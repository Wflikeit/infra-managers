...
func TestRmtAccessInventoryClient_GetRemoteAccessConf(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		got, err := rmtAccessCfgClient.GetRemoteAccessConf(ctx, tenantID, "remoteaccess-deadbeef", time.Second)
		require.Error(t, err)
		require.Nil(t, got)
	})
	...
	t.Run("ok", func(t *testing.T) {
		...
		got, err := rmtAccessCfgClient.GetRemoteAccessConf(ctx, tenantID, rac.GetResourceId(), time.Second)
		require.NoError(t, err)
